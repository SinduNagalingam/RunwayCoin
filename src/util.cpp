...
const char * const BITCOIN_CONF_FILENAME = "runwaycoin.conf";
const char * const BITCOIN_PID_FILENAME = "runwaycoind.pid";
...
#else
    const char* pszModule = "runwaycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "runwaycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/runwaycoin";
#else
    // Unix
    return pathRet / ".runwaycoin";
#endif
#endif
}
